...
export async function createHandler(request: FastifyRequest<{Body: newArticleNewsInput}>, reply: FastifyReply) {
    const body = request.body;
    try {
     
        const news = await create(body);
        return reply.code(201).send({success:true, message: 'Successfuly Created', data: news});
      } catch (e) {
...
export async function getAllHandler(request: FastifyRequest, reply: FastifyReply) {
  try {
      ...
      return reply.code(201).send({success:true, message: 'Successfuly Get All', data: news});
    } catch (e) {
      return reply.code(500).send({success:false, message: 'Server Error', error: e});
    }
}
